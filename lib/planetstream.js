...
function PlanetStream (opts) {
  ...
  var metaRedis = new Redis(port, host);
  var dataRedis = new Redis(port, host);
  var joinRedis = new Redis(port, host);
  var metaStream = MetaStream(opts);
  var dataStream = DataStream(opts); 
  var dataPipeline = dataRedis.pipeline();
  var metaPipeline = metaRedis.pipeline();
  ...
  metaStream.stream.debounce(2000).onValue(function () {
    ...
    metaPipeline = metaRedis.pipeline();
  });

  dataStream.stream.map(JSON.parse).onValue(function (data) {
    log.debug('Batching before redis data: ' + data.changeset);
  ...
  .flatten()
  .flatMap(function (id) {
    return K.fromPromise(joinRedis.exists(id).then(function (result) {
  ...
  var joinedElements = changesetInRedis.filter(function (resultTuple) {
    ...
    return K.fromPromise(joinRedis.get(changesetId).then(function (metadata) {
      return joinRedis.get('data:' + changesetId).then(function (elements) {
        var fullChangeset = {};
